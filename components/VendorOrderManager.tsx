...
export default function VendorOrderManager({ vendorUid }: { vendorUid: string }) {
  ...
  const renderNewOrder = ({ item }: { item: any }) => (
    <View style={styles.card}>
      ...
      <View style={styles.row}>
        <TouchableOpacity style={[styles.button, styles.green]} onPress={() => updateOrderStatus(item.id, 'ACCEPTED')}>
          <Text style={styles.buttonText}>Accept</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.button, styles.red]} onPress={() => updateOrderStatus(item.id, 'CANCELLED')}>
  ...
  const renderActiveOrder = ({ item }: { item: any }) => (
    <View style={styles.card}>
      <Text style={styles.title}>Delivery: {item.deliveryAddress || 'N/A'}</Text>
      <Text>Phone: {item.customer?.phone || 'N/A'}</Text>
      <View style={styles.row}>
        <TouchableOpacity style={[styles.button, styles.green]} onPress={() => Linking.openURL(`tel:${item.customer?.phone}`)}>
          <Text style={styles.buttonText}>Call Customer</Text>
        </TouchableOpacity>
      </View>
      {item.status === 'ACCEPTED' && (
        <TouchableOpacity style={[styles.button, styles.green]} onPress={() => updateOrderStatus(item.id, 'DISPATCHED')}>
          <Text style={styles.buttonText}>Dispatch Order</Text>
        </TouchableOpacity>
      )}
      {item.status === 'DISPATCHED' && (
        <TouchableOpacity style={[styles.button, styles.green]} onPress={() => updateOrderStatus(item.id, 'DELIVERED')}>
    ...
    <View style={styles.container}>
      ...
      {tab === 'new' ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.empty}>No ongoing orders.</Text>}
        />
      )}
    </View>
  );
